...
export class MediaDbPreviewModal extends Modal {
	...
	async preview(): Promise<void> {
		...
		for (const result of this.elements) {
			...
			const fileContent = this.plugin.noteManager.generateMediaDbNoteFrontmatterPreview(result);
			const pre = fileDiv.createEl('pre');
			pre.createEl('code', { text: fileContent, cls: 'language-yaml' });

			const importPath = this.plugin.noteManager.getResolvedImportPath(result);
