...
export class PropertyMapper {
	...
	convertObjectBack(obj: Record<string, unknown>): Record<string, unknown> {
		...
		for (const model of models) {
			...
			const typeKey = typePm?.mapping === PropertyMappingOption.Map && typePm.newProperty ? typePm.newProperty : 'type';
			...
			if (typeStr === (model.type as string) || typeStr === noteTypeValueForMedia(this.plugin.settings, model.type)) {
				matchedModel = model;
				break;
			}
		}
		...
		objLoop: for (const [key, value] of Object.entries(obj)) {
			...
			for (const propertyMapping of propertyMappings) {
				if (propertyMapping.mapping === PropertyMappingOption.Map && propertyMapping.newProperty === key) {
