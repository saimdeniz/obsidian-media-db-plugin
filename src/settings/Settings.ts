...
export interface MediaDbPluginSettings {
	...
	ignoredBodySectionsOnUpdate: string;
	activeCreatorCommands: MediaType[];
	preservePropertyOrderOnUpdate: boolean;
...
const DEFAULT_SETTINGS: MediaDbPluginSettings = {
	...
	ignoredBodySectionsOnUpdate: '',
	activeCreatorCommands: [MediaType.Game, MediaType.Book, MediaType.Movie, MediaType.Series],
	preservePropertyOrderOnUpdate: true,
...
export class MediaDbSettingTab extends PluginSettingTab {
				...
				group.addSetting(
					setting =>
						void setting
							...
							.setDesc('Store Budget and Revenue as nested objects with numeric value and currency (e.g. USD) instead of a formatted string.')
							.addToggle(cb => {
	...
	display(): void {
		...
		addTab('advanced', 'Advanced', 'settings-2', panel => {
			const advancedGroup = new SettingGroup(panel);

			advancedGroup.addSetting(
				setting =>
					void setting
						.setName('Preserve property order during update')
						.setDesc('If enabled, updating a note preserves the order of existing YAML properties. If disabled, properties will be reordered to match the template.')
						.addToggle(cb => {
							cb.setValue(this.plugin.settings.preservePropertyOrderOnUpdate).onChange(data => {
								this.plugin.settings.preservePropertyOrderOnUpdate = data;
								void this.plugin.saveSettings();
							});
						}),
			);
			...
			advancedGroup.addSetting(
				setting =>
					void setting
						.setName('Ignored body sections on update')
						.setDesc('Comma-separated list of H2 heading titles to protect during note body updates (e.g. Notes, Review).')
						.addText(cb => {
							cb.setPlaceholder('Notes, Review')
								.setValue(this.plugin.settings.ignoredBodySectionsOnUpdate)
								.onChange(data => {
									this.plugin.settings.ignoredBodySectionsOnUpdate = data;
									void this.plugin.saveSettings();
								});
						}),
			);
			...
			autoTrackerGroup.addSetting(
				setting =>
					void setting
						.setName('Smart Update "Airing" Property')
						.setDesc('Property key to check if a TV show is currently airing (used by Smart Update). Default is "airing".')
						.addText(text => {
			...
			autoTrackerGroup.addSetting(
				setting =>
					void setting
						.setName('Smart Update "Released" Property')
						.setDesc('Property key to check if a media item is unreleased (used by Smart Update). Default is "released".')
						.addText(text => {
			...
			const commandGroups = [
				...
				{ name: 'Music', types: [MediaType.Artist, MediaType.MusicRelease, MediaType.Song] },
				{ name: 'Books', types: [MediaType.Book, MediaType.ComicManga] },
				{ name: 'Games', types: [MediaType.Game, MediaType.BoardGame] },
				{ name: 'Other', types: [MediaType.Wiki] },
			];

			const creatorGroup = new SettingGroup(panel);
			const singleCard = creatorGroup.listEl.createDiv({ cls: 'media-db-creator-card-content' });

			for (const group of commandGroups) {
				const row = singleCard.createDiv({ cls: 'media-db-creator-row' });
				row.createEl('div', { text: group.name, cls: 'media-db-creator-row-label' });

				const pillsRow = row.createDiv({ cls: 'media-db-creator-row-pills' });

				for (const mediaType of group.types) {
					const isActive = this.plugin.settings.activeCreatorCommands.includes(mediaType);
					const pill = pillsRow.createDiv({ cls: 'media-db-creator-pill' });
					if (isActive) {
						pill.addClass('is-active');
					}

					const iconContainer = pill.createSpan();
					setIcon(iconContainer, mediaTypeTabIcon(mediaType));

					pill.createSpan({ text: unCamelCase(mediaType) });

					pill.addEventListener('click', async () => {
						const index = this.plugin.settings.activeCreatorCommands.indexOf(mediaType);
						if (index > -1) {
							this.plugin.settings.activeCreatorCommands.splice(index, 1);
						} else {
							this.plugin.settings.activeCreatorCommands.push(mediaType);
						}
						await this.plugin.saveSettings();

						this.plugin.updateCreatorCommands();
						this.display();
					});
				}
			}
