 ...
 */
export function releaseGroupMatchesPrimaryTypeImportFilter(primaryType: string | undefined, enabled: ReleaseGroupPrimaryTypes): boolean {
	if (!primaryType) {
		return false;
	}
	for (const t of MUSICBRAINZ_RELEASE_GROUP_PRIMARY_TYPES) {
...
export type InferredReleaseGroupSecondaryTypeId = (typeof INFERRED_RELEASE_GROUP_SECONDARY_TYPES)[number]['id'];
...
export function normalizeReleaseGroupSecondaryTypes(raw: Record<ReleaseGroupSecondaryTypeId, boolean> | undefined): Record<ReleaseGroupSecondaryTypeId, boolean> {
 ...
 */
function releaseGroupPassesMusicBrainzSecondaryTypeFilter(secondaryTypes: string[] | undefined, allowed: Record<ReleaseGroupSecondaryTypeId, boolean>): boolean {
